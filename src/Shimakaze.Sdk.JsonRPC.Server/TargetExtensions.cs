...
public static partial class TargetExtensions
{
    ...
    private static string Combine(string? s1, string? s2)
    {
        ...
        string result = s1.EndsWith('/') && s2.StartsWith('/')
            ? s1[..^1] + s2
            : !s1.EndsWith('/') && !s2.StartsWith('/')
            ? s1 + '/' + s2
            : s1 + s2;
    ...
    private static string TrimAsyncTail(string path) => path.EndsWith("Async", StringComparison.OrdinalIgnoreCase) ? path[..^5] : path;
