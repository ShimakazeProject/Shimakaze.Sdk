...
/// 表示一个 INI 编译器指令的语法节点。
/// </summary>
public sealed class CompilerCommandSyntaxNode : SyntaxNode
{
    /// <summary>
    /// 初始化一个新的 <see cref="CompilerCommandSyntaxNode"/> 实例。
    /// </summary>
    /// <param name="green">对应的绿树节点。</param>
    /// <param name="parent">当前节点的父节点，若为根节点则为 null。</param>
    internal CompilerCommandSyntaxNode(CompilerCommandNode green, SyntaxNode? parent)
        : base(green, parent)
    {
    }
    ...
    /// 获取与此红树节点关联的绿树节点。
    /// </summary>
    internal new CompilerCommandNode Green => (CompilerCommandNode)base.Green;

    /// <summary>
    /// 获取当前编译器指令的词法单元（Token），包含原始文本及其位置信息。
    /// </summary>
    public IniToken Token => Green.Token;

    /// <summary>
    /// 获取一个值，指示此节点是否为叶子节点（即不包含子节点的终端节点）。
    /// 对于 <see cref="CompilerCommandSyntaxNode"/>，始终返回 true。
    /// </summary>
